...
const Home: NextPage = () => {
  ...
  const { data: isLotteryOperator} = useContractRead(contract, "isLotteryOperator");
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      console.error("contract call failre", err);
    ...
    <div className="bg-[#091818] min-h-screen flex flex-col">
        ...
        {winnings > 0 && (
          <div className='max-w-md md:max-w-2xl lg:max-w-4xl mx-auto mt-5'>
            <button onClick={onWithdrawWinnings} className='p-5 bg-gradient-to-b from-orange-500 to-emerald-600 animate-pulse text-center rounded-xl w-full'>
              <p className='font-bold'> Winner Winner Mutton Dinner</p>
      ...
      <div className='space-y-5 md:space-y-0 m-5 md:flex md:flex-row items-start justify-center md:space-x-5'>
        ...
        <div className="stats-container spce-y-2">
          <div className="state-container">
            ...
            <button disabled={expiration?.toString() < Date.now().toString()
            ...
            <div className='stats'>
              ...
              <div className='flex max-w-sm flex-wrap gap-x-2 gap-y-2'>
                {Array(userTickets).fill("").map((_, index) => (
                  <p key={index} className="text-emerald-300 h-20 w-12 bg-emerald-500/30 rounded-lg flex flex-shrink-0 items-center justify-center text-sm italic">
                    {index + 1}
                    </p>
                ))}
            </div>
      ...
      <footer className="border-t border-emerald-500/20 flex items-center text-white justify-between p-5">
        <img className='h-10 w-10 filter hue-rotate-90 opacity-20 rounded-full'
         src="https://i.pinimg.com/564x/16/90/78/1690786f15d24c19e68d05ddf6308183.jpg" 
         alt=""/>

         <p className='text-xs text-emerald-900 pl-5'>
          DISCLAIMER: Play to earn. Dont blame us for your buying spree and getting addicted to this LOTTERY DRAW by VENU
         </p>
      </footer>
